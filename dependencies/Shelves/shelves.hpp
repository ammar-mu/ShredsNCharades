...
//    T* k1 = (T*)_mm_malloc(len * sizeof(T), 16);
//    T* k2 = (T*)_mm_malloc(len * sizeof(T), 16);
//    T* yi = (T*)_mm_malloc(len * sizeof(T), 16);
//
//    f(y, k1);
//
//    for (int i = 0; i < len; i++) {
...
public:
    LPFilter() 
    {   voltage_ = (T*) _mm_malloc(sizeof(T), 16); // Dynamically allocate aligned memory
    }

    ~LPFilter() 
    {   _mm_free(voltage_); // Free allocated memory
    }
...
protected:
    T* voltage_; // Pointer to aligned memory
};
...
public:
    SVFilter() {
        voltage_ = (T*) _mm_malloc(3 * sizeof(T), 16); // Dynamically allocate aligned memory for 2 elements
        lp_ = voltage_; bp_ = &(voltage_[1]); hp_ = &(voltage_[2]); 
    }
...
protected:
    T* voltage_; // Aligned memory for voltage
    T *lp_;
    T *bp_;
    T *hp_;
};
...
public:
    struct Frame
    {
        ...
        // Options
        float pre_gain; // Ammar: Changed from bool to float.  
                        // original: True = -6dB, False = 0dB
    };
    ...
    void process(Frame& frame)
    {
        ...
        // q_cv[0] = frame.main_in * (frame.pre_gain ? 0.25f : 0.5f);  // Original
        q_cv[0] = frame.main_in * frame.pre_gain;   // Ammar: Changed to a float value controled by a knob!!
